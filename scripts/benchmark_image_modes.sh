...
count_expected_events() {
  jq -c 'if .expectedEvents != null then .expectedEvents else [.expectedEvent] end'
}
...
score_case() {
  ...
  if jq -e '.scheduleRows != null' >/dev/null <<<"$case_json"; then
    if [[ "$case_exact" == "1" && "$expected_count" -gt 1 ]]; then
      row_case_exact=1
    fi
  fi
...
jq -c '.cases[]' "$MANIFEST" | while read -r case_json; do
  ...
  expected_events="$(count_expected_events <<<"$case_json")"
  ocr_text="$(ocr_text_for_case "$case_json")"
  ...
  classic_payload="$(classic_prompt "$datetime" "$timezone" "$language")"
  jq -n --arg model "$MODEL" --arg prompt "$classic_payload" --rawfile img "$img_b64_file" '{
    model: $model,
  ...
  heavy_obs_prompt="$(heavy_observation_prompt "$datetime" "$timezone" "$language" "$ocr_text")"
  jq -n --arg model "$MODEL" --arg prompt "$heavy_obs_prompt" --rawfile img "$img_b64_file" '{
    model: $model,
    messages: [{role: "user", content: $prompt, images: [$img]}],
    stream: false,
    format: "json",
  ...
  heavy_temp_prompt="$(heavy_temporal_prompt "$datetime" "$timezone" "$language" "$parsed_observations")"
  jq -n --arg model "$MODEL" --arg prompt "$heavy_temp_prompt" '{
    model: $model,
...
}
{
  ...
  row_exact_hits[mode] += $10
  parse_failures[mode] += $11
  if ($10 == 1) row_cases[mode]++
}
END {
  for (mode in cases) {
    ...
    if (row_cases[mode] > 0) {
      printf(" row_exact=%s%%", pct(row_exact_hits[mode], row_cases[mode]))
    }
    printf("\n")
  }
